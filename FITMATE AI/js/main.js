...
function initDownloadHandlers() {
  ...
  downloadButtons.forEach((btn) => {
    btn.addEventListener('click', (e) => {
      ...
      const platformName = platform === 'android' ? 'Android (FITMATE-AI.apk)' : 'Windows (FITMATE-AI-Setup.exe)';

      // Trigger user confirmation feedback toast immediately
      showToast(
        'Your download is starting…',
        `Preparing ${filename}. Once downloaded, run the installer to begin setup.`
      );

      // Verify file presence before triggering download
      fetch(filepath, { method: 'HEAD' })
        .then((response) => {
          if (response.ok) {
            // Initiate standard browser download with explicit filename
            ...
            showToast(
              'Package Preparing',
              `The ${platformName} release package is finalizing. Please check back shortly or view release checksums below.`
            );
          }
        })
        .catch(() => {
